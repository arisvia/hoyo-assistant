...
from hoyo_assistant.core.account import get_account_list, get_game_name
...
async def test_get_account_list_cookie_expired_retry():
    ...
    with (
        patch("hoyo_assistant.core.account.http.get") as mock_get,
        patch(
            "hoyo_assistant.core.account.login.update_cookie_token",
            AsyncMock(return_value=True),
        ),
        patch(
            "hoyo_assistant.core.account.config", {"account": {"cookie": "new_cookie"}}
        ),
    ):
...
async def test_get_account_list_cookie_expired_raise():
    ...
    with (
        patch(
            "hoyo_assistant.core.account.http.get", return_value=mock_response_expired
        ),
        patch(
            "hoyo_assistant.core.account.login.update_cookie_token",
